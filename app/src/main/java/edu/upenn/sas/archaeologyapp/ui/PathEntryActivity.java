...
public class PathEntryActivity extends BaseActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        locationCollector = new LocationCollector(PathEntryActivity.this, reachHost, reachPort, positionUpdateInterval) {
             ...
             * @param readLatitude - reach latitude
             * @param readLongitude - reach longitude
             * @param readAltitude - reach altitude
             * @param readStatus - reach status
             * @param readARRatio - reach AR ratio
             */
            @Override
            public void broadcastLocation(double readLatitude, double readLongitude, double readAltitude,
                                          String readStatus, Double readARRatio)
            {
                liveLatitude = readLatitude;
                liveLongitude = readLongitude;
                liveAltitude = readAltitude;
                liveStatus = readStatus;
                liveARRatio = readARRatio;
...
//        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        ...
        ArrayAdapter<String> teamMemberAdapter = new ArrayAdapter<>(this,
                android.R.layout.simple_spinner_item, teamMemberOptions);
        ...
        String teamMemberFromPreferences = settings.getString("defaultTeamMember",
                teamMembersDropdown.getItemAtPosition(0).toString());
    ...
    private void prePopulateFields()
    {
        ...
        resetStartPoint();
        resetEndPoint();
        Long passedBeginTime = getIntent().getLongExtra(Constants.PARAM_KEY_BEGIN_TIME, 0);
        String passedTeamMember = getIntent().getStringExtra(Constants.PARAM_KEY_TEAM_MEMBER);
        // If null, it means nothing was passed
        if (passedBeginTime.equals(0L) || passedTeamMember == null)
        {
            return;
        }
        // If those are passed in, we know this isn't a new path
        startPointSet = true;
        beginTime = passedBeginTime;
        teamMember = passedTeamMember;
        double passedBeginLatitude = getIntent().getDoubleExtra(Constants.PARAM_KEY_BEGIN_LATITUDE, Double.MIN_VALUE);
        double passedBeginLongitude = getIntent().getDoubleExtra(Constants.PARAM_KEY_BEGIN_LONGITUDE, Double.MIN_VALUE);
        double passedBeginAltitude = getIntent().getDoubleExtra(Constants.PARAM_KEY_BEGIN_ALTITUDE, Double.MIN_VALUE);
        String passedBeginStatus = getIntent().getStringExtra(Constants.PARAM_KEY_BEGIN_STATUS);
        double passedBeginARRatio = getIntent().getDoubleExtra(Constants.PARAM_KEY_BEGIN_AR_RATIO, Double.MIN_VALUE);
        setStartPoint(passedBeginLatitude, passedBeginLongitude, passedBeginAltitude, passedBeginStatus, passedBeginARRatio);
        ...
        for (int i = 0; i < teamMembersDropdown.getCount(); i++)
        {
            if (teamMembersDropdown.getItemAtPosition(i).toString().equalsIgnoreCase(passedTeamMember))
            {
                teamMembersDropdown.setSelection(i);
            }
        }
        // Check to see if the end point has been set before
        Long passedEndTime = getIntent().getLongExtra(Constants.PARAM_KEY_END_TIME, 0);
        if (!passedEndTime.equals(0L))
        {
            endPointSet = true;
            endTime = passedEndTime;
            double passedEndLatitude = getIntent().getDoubleExtra(Constants.PARAM_KEY_END_LATITUDE, Double.MIN_VALUE);
            double passedEndLongitude = getIntent().getDoubleExtra(Constants.PARAM_KEY_END_LONGITUDE, Double.MIN_VALUE);
            double passedEndAltitude = getIntent().getDoubleExtra(Constants.PARAM_KEY_END_ALTITUDE, Double.MIN_VALUE);
            String passedEndStatus = getIntent().getStringExtra(Constants.PARAM_KEY_END_STATUS);
            double passedEndARRatio = getIntent().getDoubleExtra(Constants.PARAM_KEY_END_AR_RATIO, Double.MIN_VALUE);
            setEndPoint(passedEndLatitude, passedEndLongitude, passedEndAltitude, passedEndStatus, passedEndARRatio);
        }
    }

    /**
     * Set the start point
     * @param passedLatitude - latitude
     * @param passedLongitude - longitude
     * @param passedAltitude - altitude
     * @param passedStatus - status
     * @param passedARRatio - AR ratio
     */
    private void setStartPoint(double passedLatitude, double passedLongitude, double passedAltitude,
                               String passedStatus, Double passedARRatio)
    {
        ...
        beginLongitude = passedLongitude;
        beginLatitude = passedLatitude;
        beginAltitude = passedAltitude;
        beginStatus = passedStatus;
        beginARRatio = passedARRatio;
     ...
     * @param passedARRatio - AR ratio
     */
    private void setEndPoint(double passedLatitude, double passedLongitude, double passedAltitude,
                             String passedStatus, Double passedARRatio)
    {

        // Change "Start path" button to "End path", populate starting point coords and begin time
        endLongitude = passedLongitude;
        endLatitude = passedLatitude;
        endAltitude = passedAltitude;
        endStatus = passedStatus;
        endARRatio = passedARRatio;
        // Update the text views for coordinates
    ...
    public void submitButtonPressed(View v)
    {
        ...
        if (liveLatitude != null && liveLatitude != 0)
        {
            if (!startPointSet)
            {
